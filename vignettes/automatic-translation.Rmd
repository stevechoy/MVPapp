...
`r "\u26A0\uFE0F"` Multiple files are supported, as long as they have the same extension (e.g. all PDF files). For example, if the model descriptions are in the main body of the article, while the model equations and/or parameters are provided in the supplementary files, simply upload them all at the same time.  
...
MVP (>= v0.4.1) supports deterministic translation (i.e. not using LLMs) of NONMEM files (`.ctl/.mod/.lst`), courtesy of the [nonmem2mrgsolve](https://github.com/Andy00000000000/nonmem2mrgsolve){target="_blank"} and [nonmem2rx](https://github.com/nlmixr2/nonmem2rx){target="_blank"} R packages.  
...
When either packages are installed, new checkbox options would be available during customization (default not used). When checked, deterministic translation will be triggered when:  
  ...
  - nonmem2rx - If the user uploads **EXACTLY** 1 file: the NONMEM (`.ctl/.mod/.lst`) file.  
