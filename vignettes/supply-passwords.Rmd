...
A list of models (stored as character objects) are included by default when `MVPapp` is installed. This is the list that the users sees in the drop-down list when they select models, and is stored as a vector of characters called `model_examples_list`. There are two functions related to model selection:  
